function addNote () {
    ...
    $('.val-btn').click(function() {
        ...
        if (numTabforNotes != 1) {
            ...
            for (let x = 0; x < noteMemory.length; x++) {
                ecartGen += ((parseInt(noteMemory[x]) - genMoy)**2)*parseInt(coefMemory[x]);
            
            }
            ecartGen /= genMoyCoef
            ecartGen = Math.sqrt(ecartGen);
            $('.moy-result p').text(Math.round(genMoy*100)/100);
            $('.ecart-result p').text(Math.round(ecartGen*100)/100)
            $('.result-moy-and-ecart').removeClass('d-none');
            $('.form-moyenne').addClass('d-none');
            $('.div-to-export').addClass('bg-light');

        }
        $('.btn-back-to-enter-nb').click(function() {
            $('.result-moy-and-ecart').addClass('d-none');
            $('.form-moyenne').removeClass('d-none');
            $('.div-to-export').removeClass('bg-light');
        })

        $('.btn-export').click(function() {
            domtoimage.toJpeg(document.getElementById('export'), { quality: 0.95 })
